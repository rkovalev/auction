...
public class AuctionDiscrete {
    ...
    public AuctionDiscrete(String ticker, int ordersLimit, int minPriceRub, int maxPriceRub, int maxSizeLimit) {
        ...
        sumOrderQtysBuySide = new int[maxPriceRub * 100];
        sumOrderQtysSellSide = new int[maxPriceRub * 100];
    ...
    private void findPriceAndSize() {
        ...
        int intersectedBuySize = 0, intersectedSellSize = 0;
        for(int i = minSellIdx; i <= maxBuyIdx; i++) {
            intersectedBuySize += sumOrderQtysBuySide[i];
            intersectedSellSize += sumOrderQtysSellSide[i];
        }

        if(intersectedBuySize > intersectedSellSize) {
            auctionSize = intersectedSellSize;
            auctionPrice = 0.01d * minSellIdx;
        } else {
            auctionSize = intersectedBuySize;
            auctionPrice = 0.01d * maxBuyIdx;
        }
    }
